...
class bank_acc_rec_statement(orm.Model):
    ...
    def check_group(self, cr, uid, ids, context=None):
        ...
        group_verifier_id = model_data_obj._get_id(
            cr, uid,
            'account_banking_reconciliation_optimization', 'group_bank_stmt_verifier'
        )
...
#        )
    ...
    def refresh_record(self, cr, uid, ids, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context=context):
            ...
            obj.write(to_write)
        for line in obj.credit_move_line_ids + obj.debit_move_line_ids:
            if line.move_line_id and line.move_line_id.move_id.state != 'posted':
                line.unlink()
                continue
            sql_statement = """ select * from bank_acc_rec_statement_line bal
                        inner join bank_acc_rec_statement ba ON bal.statement_id = ba.id
                        where bal.move_line_id = %d and bal.cleared_bank_account = True 
                        and bal.statement_id != %d and ba.state != 'cancel' """ % (line.move_line_id.id, ids[0])
            cr.execute(sql_statement)
            statement_line_ids = cr.dictfetchall()
    ...
    def is_b_a_r_s_state_done(self, cr, uid, move_line_id, context=None):
        ...
        statement_line_ids = statement_line_obj.search(
            cr, uid, [('move_line_id', '=', move_line_id), ('cleared_bank_account', '=', True)], context=context
        )
    ...
    def onchange_account_id(
        ...
        if len(ids) == 1:
            statement_obj = self.browse(cr, uid, ids[0], context=context)
            if 'multi_currency' not in val['value']:
                val['value']['multi_currency'] = statement_obj.multi_currency
            if 'name' not in val['value']:
                val['value']['name'] = statement_obj.name

            if 'starting_balance_in_currency' not in val['value']:
                val['value']['starting_balance_in_currency'] = statement_obj.starting_balance_in_currency
            if 'ending_balance_in_currency' not in val['value']:
                val['value']['ending_balance_in_currency'] = statement_obj.ending_balance_in_currency
            if 'company_id' not in val['value']:
                val['value']['company_id'] = statement_obj.company_id.id
            
            if 'account_id' not in val['value']:
                val['value']['account_id'] = account_id
            if 'ending_date' not in val['value']:
                val['value']['ending_date'] = ending_date
            if 'suppress_ending_date_filter' not in val['value']:
                val['value']['suppress_ending_date_filter'] = suppress_ending_date_filter
                statement_obj.write({'suppress_ending_date_filter': suppress_ending_date_filter})
            if 'keep_previous_uncleared_entries' not in val['value']:
                val['value']['keep_previous_uncleared_entries'] = keep_previous_uncleared_entries
                statement_obj.write({'keep_previous_uncleared_entries': keep_previous_uncleared_entries})
            if 'currency_rate' not in val['value']:
                val['value']['currency_rate'] = statement_obj.currency_rate
